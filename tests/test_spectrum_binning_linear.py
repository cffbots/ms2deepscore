...
from ms2deepscore.spectrum_binning_linear import set_slope_linear
...
def test_create_peak_list_linear():
    ...
    peak_lists, _ = create_peak_list_linear([spectrum, spectrum], class_values,
                                         min_bin_size=1.0, slope=0.0, mz_min=10.0,
                                         peak_scaling=1)
...
def test_set_slope_linear():
    slope = set_slope_linear(1000, min_bin_size=0.01, mz_min=10.0, mz_max=100.0)
    assert slope == pytest.approx(0.00016016016, 1e-6), "Expected different result."
...
def test_unique_peaks_linear():
    ...
    class_values, unique_peaks = unique_peaks_linear([spectrum, spectrum], 
                                                     min_bin_size=0.01, slope=0.001, mz_min=10.0)
...
def test_bin_number_array_linear():
    ...
    intensities = np.array([1, 1, 1, 1, 0.5], dtype="float")
    spectrum = Spectrum(mz=mz, intensities=intensities)
    bins = bin_number_array_linear(spectrum.peaks.mz, min_bin_size=0.01, slope=0.001, mz_min=10.0)
