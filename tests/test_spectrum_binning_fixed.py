...
from ms2deepscore.spectrum_binning_fixed import set_bin_size_fixed
...
def test_set_bin_size_fixed():
    bin_size = set_bin_size_fixed(1000, mz_min=10.0, mz_max=100.0)
    assert bin_size == 0.09, "Expected different result (0.09)."
...
def test_unique_peaks_fixed():
    ...
    class_values, unique_peaks = unique_peaks_fixed([spectrum, spectrum], bin_size=0.09,
...
def test_bin_number_array_fixed():
    ...
    spectrum = Spectrum(mz=mz, intensities=intensities)
    bins = bin_number_array_fixed(spectrum.peaks.mz, bin_size=0.1, mz_max=100.0, mz_min=10.0)
